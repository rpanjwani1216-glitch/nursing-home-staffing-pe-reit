...
TABLE_DIR = ROOT / "outputs" / "generated" / "tables" / "econometrics"
FIG_DIR = ROOT / "outputs" / "generated" / "figures" / "econometrics"
PANEL_MAIN = ROOT / "data" / "analysis" / "_stata" / "pe_national.dta"
...
STAR_EVENT_FILES = [
    ("csdid_event_health_rating_gold_plus_silver.csv", "health_rating_gold_plus_silver", "Health rating"),
    ("csdid_event_overall_rating_gold_plus_silver.csv", "overall_rating_gold_plus_silver", "Overall rating"),
    ("csdid_event_staffing_rating_gold_plus_silver.csv", "staffing_rating_gold_plus_silver", "Staffing rating"),
    ("csdid_event_qm_rating_gold_plus_silver.csv", "qm_rating_gold_plus_silver", "QM rating"),
]
...
def build_overall_pretrend_summary() -> None:
    mappings = {
        "RN HPRD": parse_esttab_csv(TABLE_DIR / "csdid_event_rn_gold_plus_silver.csv")[1],
        "Total nurse HPRD": parse_esttab_csv(TABLE_DIR / "csdid_event_total_nurse_gold_plus_silver.csv")[1],
    }
    rows = []
    ...
    write_table(table_df, "paper_pretrend_summary_gold_plus_silver", "Pre-treatment event-study summary")


def build_overall_event_plot_data() -> None:
    for stem, out_stem in [
        ("csdid_event_total_nurse_gold_plus_silver.csv", "csdid_event_total_nurse_gold_plus_silver_plot_data"),
        ("csdid_event_rn_gold_plus_silver.csv", "csdid_event_rn_gold_plus_silver_plot_data"),
    ]:
...
def build_star_pretrend_summary() -> None:
    ...
    for filename, _, outcome_label in STAR_EVENT_FILES:
        ...
        for term in TERM_ORDER_PRE:
            ...
            rows.append(row)
    table_df = pd.DataFrame(rows)
    write_table(table_df, "paper_star_pretrend_summary_gold_plus_silver", "Pre-treatment event-study summary: star ratings")
...
def build_leave_one_deal_table() -> None:
    loo = pd.read_stata(TABLE_DIR / "leave_one_deal_out_raw_gold_plus_silver.dta")
    baseline = pd.read_stata(TABLE_DIR / "paper_main_coef_plot_data_gold_plus_silver.dta")
    total = loo[loo["outcome"] == "Total nurse HPRD"].copy()
    deal_size_lookup = total.groupby("omitted_deal")["omitted_facilities"].max().to_dict()
    row_order = ["All deals"] + [f"Exclude {deal} ({int(deal_size_lookup[deal])} homes)" for deal in sorted(deal_size_lookup)]
    main_panel = pd.read_stata(PANEL_MAIN, columns=["ccn_str", "sample_role", "sample_main_staffing"])
    treated_total = int(
        main_panel.loc[
            (main_panel["sample_role"] == "treated") & (main_panel["sample_main_staffing"] == 1),
            "ccn_str",
        ].nunique()
    )
    rows = []
    ...
    rows.append(
        {
            ...
            "Treated_facilities_left": treated_total,
        }
    )
    for deal in sorted(deal_size_lookup):
        n_homes = int(deal_size_lookup[deal])
    ...
    write_table(table_df, "paper_leave_one_deal_out_total_nurse_gold_plus_silver", "Leave-one-deal-out robustness: total nurse staffing")


def build_medicaid_pretrend_summary() -> None:
    _, df = parse_esttab_csv(TABLE_DIR / "csdid_event_total_nurse_high_low_gold_plus_silver.csv")
    ...
    for term in TERM_ORDER_PRE:
        ...
        rows.append(row)
    table_df = pd.DataFrame(rows)
    write_table(table_df, "paper_medicaid_pretrend_summary_gold_plus_silver", "Medicaid-group pretrend diagnostics")
...
def build_medicaid_pretrend_plot_data() -> None:
    ...
    df["lb"] = df["coef"] - 1.96 * df["se"]
    df["ub"] = df["coef"] + 1.96 * df["se"]
    write_data(df[["panel", "group", "term", "term_order", "coef", "se", "lb", "ub"]], "paper_medicaid_pretrend_plot_data_gold_plus_silver")


def build_medicaid_functional_form_summary() -> None:
    _, cont = parse_esttab_csv(TABLE_DIR / "staffing_heterogeneity_total_nurse_gold_plus_silver.csv")
    _, split = parse_esttab_csv(TABLE_DIR / "staffing_high_low_total_nurse_gold_plus_silver.csv")
    _, terc = parse_esttab_csv(TABLE_DIR / "staffing_medicaid_terciles_total_nurse_gold_plus_silver.csv")

    rows = []
    cont_df = cont[
        cont["term"].str.contains("did_treat", regex=False)
        & cont["term"].str.contains("baseline_medicaid_centered", regex=False)
    ...
    table_df = pd.DataFrame(rows)
    write_table(table_df, "paper_medicaid_functional_form_summary_gold_plus_silver", "Functional-form checks for Medicaid heterogeneity")


def build_medicaid_tercile_plot_data() -> None:
    _, df = parse_esttab_csv(TABLE_DIR / "staffing_medicaid_terciles_total_nurse_gold_plus_silver.csv")
    df = df[df["term"] == "did_treat"].head(9).copy()
    labels = [
        ("National", "Low"), ("National", "Middle"), ("National", "High"),
        ("Same-state", "Low"), ("Same-state", "Middle"), ("Same-state", "High"),
        ("Matched", "Low"), ("Matched", "Middle"), ("Matched", "High"),
    ]
    if len(df) != len(labels):
        raise ValueError(f"Expected 9 PE tercile estimates; found {len(df)}")
    df[["panel", "tercile"]] = pd.DataFrame(labels, index=df.index)
    df["lb"] = df["coef"] - 1.96 * df["se"]
    df["ub"] = df["coef"] + 1.96 * df["se"]
    write_data(
        df[["panel", "tercile", "coef", "se", "lb", "ub"]],
        "paper_medicaid_terciles_total_nurse_plot_data_gold_plus_silver",
    )
...
def build_medicaid_descriptive_scatter_data() -> None:
    ...
    write_data(plot_df, "paper_baseline_medicaid_vs_staffing_change_data_gold_plus_silver")


def build_wildbootstrap_summary() -> None:
    df = pd.read_stata(TABLE_DIR / "wildbootstrap_raw_gold_plus_silver.dta")
    rows = []
    for outcome in ["Total nurse HPRD", "RN HPRD"]:
        row = {"Outcome": outcome}
        sub = df[df["outcome"] == outcome].copy()
        for panel in PANEL_ORDER:
            obs = sub[sub["panel"] == panel]
            if obs.empty:
                row[panel] = ""
                row[f"{panel}_wild_p"] = np.nan
                row[f"{panel}_clusters"] = np.nan
                row[f"{panel}_reps"] = np.nan
                continue
            rec = obs.iloc[0]
            row[panel] = format_estimate(rec["coef"], rec["se"])
            row[f"{panel}_wild_p"] = rec["wild_p"]
            row[f"{panel}_clusters"] = int(rec["n_clusters"]) if pd.notna(rec["n_clusters"]) else np.nan
            row[f"{panel}_reps"] = int(rec["n_reps"]) if pd.notna(rec["n_reps"]) else np.nan
        rows.append(row)
    write_table(pd.DataFrame(rows), "paper_wildbootstrap_summary_gold_plus_silver", "Wild-bootstrap sensitivity with deal/control clustering")
...
def main() -> None:
    ...
    build_medicaid_tercile_plot_data()
    build_medicaid_descriptive_scatter_data()
    build_wildbootstrap_summary()
    if (TABLE_DIR / "paper_star_main_coef_plot_data_gold_plus_silver.dta").exists():
