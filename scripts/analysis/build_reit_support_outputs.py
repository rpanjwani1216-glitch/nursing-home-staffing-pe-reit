...
TABLE_DIR = ROOT / "outputs" / "generated" / "tables" / "econometrics_reit"
...
def build_medicaid_tercile_plot_data() -> None:
    _, df = parse_esttab_csv(TABLE_DIR / "reit_staffing_medicaid_terciles_directcare.csv")
    ...
    if len(df) != len(labels):
        raise ValueError(f"Expected 9 REIT tercile estimates; found {len(df)}")
    df[["panel", "tercile"]] = pd.DataFrame(labels, index=df.index)
    df["lb"] = df["coef"] - 1.96 * df["se"]
    df["ub"] = df["coef"] + 1.96 * df["se"]
    write_data(
        df[["panel", "tercile", "coef", "se", "lb", "ub"]],
        "paper_medicaid_terciles_directcare_reit_plot_data",
    )
