#!/usr/bin/env python3
"""Build the main regression-ready facility-year analysis panel.

This script prepares the Python-side handoff to Stata for the main design:

- treatment sample: gold_main
- control sample: v2
- heterogeneity source: LTCFocus baseline Medicaid dependence
- primary outcomes emphasized: staffing, ratings, and payer mix

Deficiency outcomes are intentionally omitted from this panel for now because
the current project decision is to defer deficiency analysis.
"""
...
CONTROL_V2_PATH = CMS_DIR / "clean_control_candidates_v2.csv"
TREATMENT_PATH = RESEARCH_DIR / "pe_facility_verification.csv"
BASELINE_PATH = LTCFOCUS_DIR / "ltcfocus_medicaid_baseline_inputs.csv"

OUTPUT_CSV = PANELS_DIR / "regression_analysis_panel_gold_v2.csv"
OUTPUT_DTA = PANELS_DIR / "regression_analysis_panel_gold_v2.dta"
SUMMARY_JSON = PANELS_DIR / "regression_analysis_panel_gold_v2_summary.json"
...
def load_controls_v2() -> pd.DataFrame:
...
def build_panel() -> pd.DataFrame:
    ...
    treated_universe["analysis_sample"] = "gold_main"
    ...
    panel["analysis_spec"] = "gold_main_v2"
    panel["analysis_sample_label"] = panel["sample_role"].map(
        {"treated": "gold_main", "control": "control_v2"}
    )
    ...
    # Medicare-heavy flag: top quartile of facility-level baseline Medicare share.
    # Use the unique-facility baseline distribution rather than year-weighted panel
    # rows so the cutoff is stable and interpretable.
    ...
    panel["has_star_rating_outcome"] = (
        panel["provider_health_rating"].notna().astype(int)
    )
    ...
    if panel.duplicated(["ccn_str", "year"]).any():
        raise ValueError("Regression analysis panel is not unique on ccn_str x year")
...
This folder contains the main Python-side regression analysis panel for the
current paper design.

Primary design encoded here:

- treatment sample: `gold_main`
- control sample: `v2`
- baseline heterogeneity: LTCFocus Medicaid share
- primary outcomes emphasized: staffing, ratings, and payer mix

Deficiency outcomes are intentionally omitted from this panel for now.

Main files:

- `regression_analysis_panel_gold_v2.csv`
- `regression_analysis_panel_gold_v2.dta`
- `regression_analysis_panel_gold_v2_summary.json`

Each row is one `ccn_str x year`.
...
Those flags are meant to make it easy to analyze different outcome families
independently in Stata without rebuilding the panel.
""",
        encoding="utf-8",
    )
