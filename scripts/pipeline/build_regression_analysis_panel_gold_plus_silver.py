#!/usr/bin/env python3
"""Build the expanded gold+silver regression-ready facility-year analysis panel."""
...
CONTROL_V2_PATH = CMS_DIR / "clean_control_candidates_v2_gold_plus_silver.csv"
...
OUTPUT_CSV = PANELS_DIR / "regression_analysis_panel_gold_plus_silver_v2.csv"
OUTPUT_DTA = PANELS_DIR / "regression_analysis_panel_gold_plus_silver_v2.dta"
SUMMARY_JSON = PANELS_DIR / "regression_analysis_panel_gold_plus_silver_v2_summary.json"
...
def load_controls() -> pd.DataFrame:
    df = pd.read_csv(CONTROL_V2_PATH, dtype={"ccn_str": "string"}, low_memory=False)
...
def build_panel() -> pd.DataFrame:
    ...
    control_universe["analysis_sample_label"] = "control_v2_gold_plus_silver"
    ...
    panel["analysis_spec"] = "gold_plus_silver_v2"
    ...
    if panel.duplicated(["ccn_str", "year"]).any():
        raise ValueError("Expanded regression analysis panel is not unique on ccn_str x year")
...
def main() -> None:
    ...
    summary = {
        "analysis_spec": "gold_plus_silver_v2",
        "rows": int(len(panel)),
