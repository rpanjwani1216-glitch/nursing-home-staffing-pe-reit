#!/usr/bin/env python3
"""Build a same-state matched REIT staffing panel after a variant search."""
...
QA_DIR = ROOT / "outputs" / "qa" / "matching" / "variants_reit"

PANEL_PATH = PANELS_DIR / "reit_staffing_panel_extended_same_state.csv"
BASELINE_FACILITY_CSV = BALANCE_DIR / "reit_baseline_facility_level_same_state.csv"
BASELINE_FACILITY_DTA = BALANCE_DIR / "reit_baseline_facility_level_same_state.dta"
PAIR_PATH = MATCHING_DIR / "matched_control_pairs_reit_same_state.csv"
CONTROL_PATH = MATCHING_DIR / "matched_control_candidates_reit_same_state.csv"
MATCHED_PANEL_CSV = PANELS_DIR / "reit_staffing_panel_extended_same_state_matched.csv"
MATCHED_PANEL_DTA = PANELS_DIR / "reit_staffing_panel_extended_same_state_matched.dta"
SUMMARY_JSON = MATCHING_DIR / "matched_control_sample_reit_same_state_summary.json"
SCOREBOARD_CSV = QA_DIR / "matching_variant_scoreboard_reit.csv"
SCOREBOARD_JSON = QA_DIR / "matching_variant_scoreboard_reit.json"
BEST_BALANCE_CSV = QA_DIR / "best_match_balance_reit.csv"
BEST_SPEC_JSON = QA_DIR / "best_match_spec_reit.json"
...
NEAREST_VARIANTS: dict[str, dict[str, object]] = {
    "nn_staffing_only": {
        "match_vars": ["ltcfocus_rn_hprd", "ltcfocus_directcare_hprd"],
        "method": "nearest_neighbor",
        "order_rule": "ccn",
    },
    "nn_size_staffing": {
        "match_vars": ["ltcfocus_total_beds", "ltcfocus_residents", "ltcfocus_rn_hprd", "ltcfocus_directcare_hprd"],
        "method": "nearest_neighbor",
        "order_rule": "ccn",
    },
    "nn_full": {
...
def build_baseline(panel: pd.DataFrame) -> pd.DataFrame:
    facility_meta = (
        panel[["ccn_str", "sample_role", "analysis_state", "facility_name_treatment", "treat_year", "year"]]
    ...
    treated_baseline = (
        treated_pre.groupby("ccn_str", as_index=False)[BALANCE_VARS]
        .mean()
        .merge(
    ...
    control_cohort_means = (
        control_long.groupby(["ccn_str", "cohort_year"], as_index=False)[BALANCE_VARS]
        .mean()
    ...
    control_baseline["valid_pseudocohorts"] = control_cohort_means.groupby("ccn_str")["cohort_year"].transform("nunique")
    ...
    for var in BALANCE_VARS:
        ...
        collapsed = tmp.groupby("ccn_str", as_index=False)["weighted_component"].sum().rename(columns={"weighted_component": var})
    ...
    baseline = baseline.merge(
        facility_meta[["ccn_str", "sample_role", "analysis_state", "facility_name_treatment", "treat_year"]],
...
def build_pairs(baseline: pd.DataFrame, spec: dict[str, object]) -> pd.DataFrame:
    ...
    for state, treated_state in treated.groupby("analysis_state", dropna=False):
        ...
        if order_rule == "hardest_first":
            ...
            for _, tr in treated_z.iterrows():
                if method == "linear_score":
                    dist = np.abs(full_ctrl["match_score"].to_numpy(dtype=float) - float(tr["match_score"]))
                else:
                    dist = nearest_distance(
                        tr[match_vars].astype(float).to_numpy(),
                        full_ctrl[match_vars].astype(float).to_numpy(),
                    )
        ...
        for _, tr in treated_z.iterrows():
            ...
            if method == "linear_score":
                dists = np.abs(available_controls["match_score"].to_numpy(dtype=float) - float(tr["match_score"]))
            else:
                dists = nearest_distance(
                    tr[match_vars].astype(float).to_numpy(),
                    available_controls[match_vars].astype(float).to_numpy(),
                )
            candidate = available_controls.assign(_distance=dists)
    ...
    return pd.DataFrame(pairs).sort_values(["analysis_state", "treated_ccn", "match_rank", "control_ccn"]).reset_index(drop=True)
...
def exhaustive_linear_variants() -> dict[str, dict[str, object]]:
    ...
    for size in range(1, len(BALANCE_VARS) + 1):
        for cols in itertools.combinations(BALANCE_VARS, size):
            slug = "_".join(col.replace("ltcfocus_", "").replace("baseline_", "").replace("_", "") for col in cols)
            name = f"linear_{slug}"
            variants[name] = {
...
def main() -> None:
    ...
    matched_controls = best_pairs[["control_ccn"]].drop_duplicates().rename(columns={"control_ccn": "ccn_str"}).assign(matched_control=1)
