#!/usr/bin/env python3
"""Build the expanded gold+silver same-state regression-ready analysis panel."""
...
CONTROL_PATH = CMS_DIR / "clean_control_candidates_v2_gold_plus_silver_same_state.csv"
TREATMENT_PATH = TREATMENT_INPUTS_DIR / "pe_facility_verification.csv"
BASELINE_PATH = LTCFOCUS_DIR / "ltcfocus_medicaid_baseline_inputs.csv"
LTCFOCUS_ANNUAL_PATH = LTCFOCUS_DIR / "ltcfocus_medicaid_analysis.csv"

OUTPUT_CSV = PANELS_DIR / "regression_analysis_panel_gold_plus_silver_v2_same_state.csv"
OUTPUT_DTA = PANELS_DIR / "regression_analysis_panel_gold_plus_silver_v2_same_state.dta"
SUMMARY_JSON = PANELS_DIR / "regression_analysis_panel_gold_plus_silver_v2_same_state_summary.json"
...
def load_treatment() -> pd.DataFrame:
    ...
    df = df[df["analysis_sample"].isin(["gold_main", "silver_main"]) & df["ccn_str"].notna()].copy()
    keep = [
        "ccn_str",
        "facility_name",
        "deal_id",
    ...
    return df.rename(
        columns={
            ...
            "city": "city_treatment",
            "analysis_sample": "treatment_analysis_sample",
        }
    )
...
def load_controls() -> pd.DataFrame:
    ...
    df = df[df["control_v2_gold_plus_silver_eligible"].eq(1) & df["ccn_str"].notna()].copy()
    keep = [
        "ccn_str",
        ...
        "control_v2_gold_plus_silver_eligible",
        "control_v2_gold_plus_silver_staffing_eligible",
        "control_v2_gold_plus_silver_deficiency_eligible",
    ]
...
def load_baseline() -> pd.DataFrame:
    ...
    df["ccn_str"] = df["ccn_str"].map(canonical_ccn).astype("string")
    df = df[df["baseline_spec"].eq("gold_plus_silver_v2") & df["ccn_str"].notna()].copy()
    if df.duplicated(["ccn_str"]).any():
        # A few treated facilities also appear in the control pseudocohort slice.
        # Prefer the treated baseline record so the merged panel uses the true pre-period window.
        sample_role_rank = {"treated": 0, "control": 1}
        df["_sample_role_rank"] = df["sample_role"].map(sample_role_rank).fillna(9)
        df = (
            df.sort_values(["ccn_str", "_sample_role_rank", "baseline_window_end"], ascending=[True, True, False])
            .drop_duplicates(subset=["ccn_str"], keep="first")
            .drop(columns=["_sample_role_rank"])
        )
    keep = [
        "ccn_str",
...
def load_annual_ltcfocus() -> pd.DataFrame:
    df = pd.read_csv(LTCFOCUS_ANNUAL_PATH, dtype={"ccn_str": "string"}, low_memory=False)
    df["ccn_str"] = df["ccn_str"].map(canonical_ccn).astype("string")
    df["year"] = pd.to_numeric(df["year"], errors="coerce").astype("Int64")
    keep = [
        "ccn_str",
        "year",
        "ltcfocus_avgadl_mds3",
        "ltcfocus_avg_dailycensus",
    ]
    keep = [c for c in keep if c in df.columns]
    df = df[df["ccn_str"].notna() & df["year"].notna()].copy()
    return df[keep].drop_duplicates(subset=["ccn_str", "year"]).copy()
...
def build_panel() -> pd.DataFrame:
    ...
    annual_ltcfocus = load_annual_ltcfocus()
    master = load_master()

    control_universe = controls[["ccn_str"]].drop_duplicates().copy()
    control_universe["sample_role"] = "control"
    control_universe["analysis_sample_label"] = "control_v2_gold_plus_silver_same_state"
    ...
    treated_universe["analysis_sample_label"] = "gold_plus_silver"
    ...
    panel = panel.merge(annual_ltcfocus, on=["ccn_str", "year"], how="left")

    panel["analysis_spec"] = "gold_plus_silver_v2_same_state"
    ...
    panel["has_provider_staffing_outcome"] = panel["provider_reported_staffing_hprd"].notna().astype(int)
    panel["has_star_rating_outcome"] = panel["provider_health_rating"].notna().astype(int)
    panel["has_form671_payer_mix_outcome"] = panel["form671_medicaid_share_mean"].notna().astype(int)
    ...
    if panel.duplicated(["ccn_str", "year"]).any():
        raise ValueError("Expanded same-state regression analysis panel is not unique on ccn_str x year")
...
def main() -> None:
    ...
    summary = {
        "analysis_spec": "gold_plus_silver_v2_same_state",
        "rows": int(len(panel)),
        ...
        "gold_treated_facilities": int(
            panel.loc[panel["treatment_analysis_sample"].eq("gold_main"), "ccn_str"].nunique()
        ),
        "silver_treated_facilities": int(
            panel.loc[panel["treatment_analysis_sample"].eq("silver_main"), "ccn_str"].nunique()
        ),
