...
global ANALYSIS_DIR "${INTERMEDIATE}/analysis"
global ANALYSIS_PANEL_DTA "${ANALYSIS_DIR}/panels/regression_analysis_panel_gold_v2.dta"
...
if !fileexists("${ANALYSIS_PANEL_DTA}") {
    di as error "Analysis panel not found: ${ANALYSIS_PANEL_DTA}"
    di as error "Run python3 scripts/build_regression_analysis_panel.py first."
    exit 601
}

log using "${QA_OUTPUT}/table1_baseline_balance.log", replace text
...
label var baseline_medicaid_share "Baseline Medicaid share"
...
preserve
    ...
    export delimited using "${QA_OUTPUT}/table1_treated_cohort_weights.csv", replace
restore

preserve
    keep if treated_ever == 1 & year < treat_year
    collapse (mean) `table_vars' (count) baseline_support_years = year, by(ccn_str treated_ever)
    label var baseline_support_years "Number of pre-treatment years used"
...
order ccn_str treated_ever baseline_support_years valid_pseudocohorts baseline_window_start baseline_window_end
sort treated_ever ccn_str
save "${ANALYSIS_BALANCE_DIR}/table1_baseline_facility_level_gold_v2.dta", replace
export delimited using "${ANALYSIS_BALANCE_DIR}/table1_baseline_facility_level_gold_v2.csv", replace
...
save "${ANALYSIS_BALANCE_DIR}/table1_baseline_balance_gold_v2.dta", replace
export delimited using "${TABLES_DIR}/table1_baseline_balance_gold_v2.csv", replace
...
tempname texhandle
file open `texhandle' using "${TABLES_DIR}/table1_baseline_balance_gold_v2.tex", write replace text
file write `texhandle' "\begin{table}[!htbp]" _n
file write `texhandle' "\centering" _n
file write `texhandle' "\caption{Baseline Comparison of Treated and Control Nursing Homes}" _n
file write `texhandle' "\label{tab:baseline_balance_gold_v2}" _n
...
file write `texhandle' "\textit{Notes:} Control means are computed from the pseudo-cohort baseline control sample aligned to the treated cohort timing. Treated means use only pre-treatment years. Differences are estimated from facility-level regressions of each baseline characteristic on a treated indicator with robust standard errors reported in parentheses." _n
file write `texhandle' "\end{minipage}" _n
file write `texhandle' "\end{table}" _n
file close `texhandle'

di as result "Baseline Table 1 artifacts created:"
di as result "  ${ANALYSIS_BALANCE_DIR}/table1_baseline_facility_level_gold_v2.dta"
di as result "  ${ANALYSIS_BALANCE_DIR}/table1_baseline_balance_gold_v2.dta"
di as result "  ${TABLES_DIR}/table1_baseline_balance_gold_v2.csv"
di as result "  ${TABLES_DIR}/table1_baseline_balance_gold_v2.tex"
