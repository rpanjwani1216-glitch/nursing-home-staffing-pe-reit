...
log using "${QA_OUTPUT}/table1_baseline_balance_same_state_matched.log", replace text
...
save "${ANALYSIS_BALANCE_DIR}/table1_baseline_facility_level_gold_plus_silver_v2_same_state_matched.dta", replace
export delimited using "${ANALYSIS_BALANCE_DIR}/table1_baseline_facility_level_gold_plus_silver_v2_same_state_matched.csv", replace
...
save "${ANALYSIS_BALANCE_DIR}/table1_baseline_balance_gold_plus_silver_v2_same_state_matched.dta", replace
export delimited using "${TABLES_DIR}/table1_baseline_balance_gold_plus_silver_v2_same_state_matched.csv", replace
...
tempname texhandle
file open `texhandle' using "${TABLES_DIR}/table1_baseline_balance_gold_plus_silver_v2_same_state_matched.tex", write replace text
file write `texhandle' "\begin{table}[!htbp]" _n
file write `texhandle' "\centering" _n
file write `texhandle' "\caption{Baseline Comparison of Treated and Matched Same-State Control Nursing Homes}" _n
file write `texhandle' "\label{tab:baseline_balance_gold_plus_silver_v2_same_state_matched}" _n
...
file write `texhandle' "\textit{Notes:} Control means are computed from the matched same-state control sample aligned to the treated cohort timing. Treated means use only pre-treatment years. Differences are estimated from facility-level regressions of each baseline characteristic on a treated indicator with robust standard errors reported in parentheses." _n
